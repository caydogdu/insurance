 ...
 *
 */
public class SportsEquipmentInsurance implements Insurance {

    private static final Logger logger = LoggerFactory.getLogger(SportsEquipmentInsurance.class);

    static final double RISK = 0.3;

    final BigDecimal minCoverage = BigDecimal.valueOf(0);

    final BigDecimal maxCoverage = BigDecimal.valueOf(20000);
        ...
        if (!(coverage.compareTo(minCoverage) > 0 && coverage.compareTo(maxCoverage) < 0)) {
            if (logger.isErrorEnabled()) {
                logger.error("coverage : " + coverage.toString() + ", range : [" + minCoverage.toString() + "-"
                        + maxCoverage + "]");
                logger.error("The coverage for Sports Equipment is not in the range");
            }
            return null;
        }
